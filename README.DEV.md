## how to use model-identity-go
The goal of model-identity-go repository is to share common data transfer object / structs definitions for other repos of the fuel HLF traceability project.

[follow the instructions](https://github.com/ic-matcom/model-identity-go/blob/main/README.md)



## DID Sample
```bash
# DID examples:
did:aa43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71
did:aa53bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71
did:aa63bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71
did:aa73bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71

# ID examples:
4a57a199-9850-45d2-a46b-f4b1d7fc9958
```
## WORLD-STATE

http://192.168.49.131:5984/_utils/

credentials: admin/adminpw

## chaincode execution samples (CLI)

### InitLedger
```bash
# populate the ledger with first data 
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:InitLedger","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
```

### populating with fake data
```bash
# populate issuer for dev (arg: none)
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:OnlyDevIssuer","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# populate identity for dev (arg: none)
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:OnlyDevParticipant","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# populate access and role for dev (arg: none)
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:OnlyDevAccess","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
 
```
### interact with the access and role transactions
```bash
# before must invoke InitLedger

# GetAccesses (arg: none)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetAccesses","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetAccess (arg: model.GetRequest)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetAccess","Args":["{\"id\":\"access-id\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# CreateRole (arg: model.RoleCreateRequest)
# before must invoke InitLedger
peer chaincode invoke -c '{"function":"org.tecnomatica.identity:CreateRole","Args":["{\"name\":\"Rol de prueba\",\"contractFunctions\":[\"GetAccess\"]}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetRoles (arg: none)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetRoles","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetRole (arg: model.GetRequest)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetRole","Args":["{\"id\":\"role-id\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# DeleteRole (arg: model.GetRequest)
peer chaincode invoke -c '{"function":"org.tecnomatica.identity:DeleteRole","Args":["{\"id\":\"role-id\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
```

### interact with the issuer transactions
```bash
# CreateIssuer (arg: model.IssuerCreateRequest)
peer chaincode invoke -c '{"function":"org.tecnomatica.identity:CreateIssuer","Args":["{\"name\":\"Autoridad de Certificación Tecnomática\",\"certPem\":\"\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetIssuerHistory (arg: model.GetRequest)
peer chaincode query  -c '{"function":"org.tecnomatica.identity:GetIssuerHistory","Args":["{\"id\":\"1d8c4f4d-ce32-4263-83cc-a76739c29469\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetIssuers  (arg: none)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetIssuers","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetIssuer (arg: model.GetRequest)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetIssuer","Args":["{\"id\":\"issuer-id\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# DeleteIssuer (arg: model.GetRequest)
peer chaincode invoke -c '{"function":"org.tecnomatica.identity:DeleteIssuer","Args":["{\"id\":\"issuer-id\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# RenewIssuer (arg: model.IssuerUpdateRequest)
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:RenewIssuer","Args":["{\"id\":\"1d8c4f4d-ce32-4263-83cc-a76739c29469\",\"certPem\":\"\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
```

### interact with the identity transactions
```bash

# CreateParticipant with role (arg: model.ParticipantCreateRequest)
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:CreateParticipant","Args":["{\"did\":\"did:ab43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec72\",\"roles\":[\"06658e98-e829-4a54-8377-8f62ac4de4b7\"],\"certPem\":\"\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetParticipant (arg: model.ParticipantGetRequest)
peer chaincode query  -c '{"function":"org.tecnomatica.identity:GetParticipant","Args":["{\"did\":\"did:ab43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec72\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetParticipants (arg: none)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetParticipants","Args":[]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetParticipantHistory (arg: model.ParticipantGetRequest)
peer chaincode query -c '{"function":"org.tecnomatica.identity:GetParticipantHistory","Args":["{\"did\":\"did:fa3bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# DeleteParticipant (arg: model.ParticipantDeleteRequest)
peer chaincode invoke  -c '{"function":"org.tecnomatica.identity:DeleteParticipant","Args":["{\"userDid\":\"did:ab43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec72\",\"callerDid\":\"did:ab43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec72\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

# GetBatch
peer chaincode query -c '{"function":"org.tecnomatica.fuelbatch:GetBatch","Args":["{\"id\":\"did:aa43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71\",\"payload\":{\"id\":\"my id\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE
```

peer chaincode invoke  -c '{"function":"org.tecnomatica.fuelbatch:Instantiate","Args":["{\"id\":\"id\",\"payload\":\"{}\",\"signature\":\"sign\"}"]}' -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA -C $CHANNEL_NAME -n $CC_NAME --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE

"Args":["{\"Id\":\"did:aa43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71\",\"Function\":\"\"}"]}'

"Args":["{\"id\":\"did:aa43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71\",\"payload\":\"\"}"]}'

'{"function":"org.tecnomatica.fuelbatch:GetBatch","Args":["{\"Id\":\"did:aa43bdf5b4bcfac88ce9093ec3f0d58290f11c7ef6d2a683a7ee56746b333ec71\",\"payload\":\"{}\"}"]}'



